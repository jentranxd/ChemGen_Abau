...
# Curated names: Which gene numbers correspond to what gene name for Ab19606 essential gene library
curated_names <- fread("curated_names.tsv")

# aba_bed: Gene locations on the chromosome (Ab19606 essential gene library)
aba_bed <- fread("CP046654.1.bed", col.names = c("chromosome", "left", "right", "locus_tag", "gene_name", "strand", "coding", "completeness"))

# Read in counts from your data
data <- fread("count_by_position.tsv.gz", header = TRUE, col.names = c("condition", "spacer", "count")) %>%
...
data$condition <- recode(data$condition, 
                         "AbJT1-0-A1T2" = "1AbJT1", "AbJT1-0-B1T2" = "1AbJT2", "AbJT1-4-2-A1T2" = "1AbJT3", "AbJT1-4-2-B1T2" = "1AbJT4", 
...
data <- data[!grepl("dJMP", data$condition)]

# aba_key: Which guides go to which gene
...
data_design <- fread("all_experimental_design.tsv", na.strings = c("NA")) %>%
  ...
  filter(condition != "4AbJT68")

data <- data %>% filter(condition != "4AbJT68")

# Set order or organize the data for accuracy in merging tables later
...
data_grid <- data.table::dcast(data, spacer ~ condition, value.var = "count", fill = 0)
...
data_design_short <- data_design %>% select("condition", "name for R script") %>%
  rename(sample1 = condition, sample_name = `name for R script`)
...
polymyxins <- data_design %>% filter(grepl('polymyxin', verbose))
...
colors <- grDevices::colorRampPalette(c("white", "navyblue"))(14)  # Example with 14 levels

# Plot for 1AbJT11 vs 1AbJT12
p1 <- ggplot(data_1AbJT, aes(x = `1AbJT11`, y = `1AbJT12`)) +
...
# Plot for 4AbJT55 vs 4AbJT56
p2 <- ggplot(data_4AbJT, aes(x = `4AbJT55`, y = `4AbJT56`)) +
  geom_density_2d_filled(alpha = 1) +
  scale_fill_manual(values = colors) +
  scale_x_log10(limits = c(1, global_max_x)) +
  scale_y_log10(limits = c(1, global_max_x)) +
